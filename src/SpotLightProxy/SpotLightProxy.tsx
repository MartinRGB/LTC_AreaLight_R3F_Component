...
import { SpotLightProxyMaterial } from './material/SpotLightProxyMaterial'
...
function VolumetricMesh({
  ...
  const [material] = React.useState(() => new SpotLightProxyMaterial())
...
// *** Copy Deafault SpotLight Attributes
...
function SpotlightShadowWithShader({
  ...
  const spotlightProxyShadowsFrag=`varying vec2 vUv;
    ...
    () =>
      new FullScreenQuad(
        new ShaderMaterial({
          ...
          `,
          fragmentShader: spotlightProxyShadowsFrag, //shader
        })
      ),
    [shader]
  )
...
const SpotLightProxy = React.forwardRef(
  (
    {
      // Volumetric
      opacity = 1,
...
SpotLightProxy.displayName='SpotLightProxy'

export { SpotLightProxy }